...
public class DataSeeder {
        ...
        public void seedData() {
                ...
                User testUser = null;
                if (userRepository.findByUsername("demo@example.com").isEmpty()) {
                        System.out.println("[DataSeeder] Creating test user...");
                        testUser = new User();
                        testUser.setFirstName("Demo");
                        testUser.setLastName("User");
                        testUser.setEmail("demo@example.com");
                        testUser.setUsername("demo@example.com");
                        testUser.setPassword(passwordEncoder.encode("Demo@123"));
                        testUser.setRoles(Set.of("USER"));
                        testUser.setEnabled(true);
                        userRepository.save(testUser);
                        System.out.println("[DataSeeder] Demo user saved.");
                        ...
                        for (int i = 0; i < WALLET_NAMES.length; i++) {
                                String walletName = WALLET_NAMES[i];
                                System.out.println("[DataSeeder] Creating wallet: " + walletName + " for test user");
                                // Set different initial balances for each wallet
                                ...
                                System.out.println("[DataSeeder] Creating " + transactionCount
                                                + " transactions for wallet: " + walletName);
                                ...
                                for (int j = 0; j < transactionCount; j++) {
                                        ...
                                        LocalDateTime date = LocalDateTime.ofEpochSecond(randomEpoch, 0,
                                        ...
                                        try {
                                                ...
                                                System.out.printf("[Transaction] %s - %s: %s (%s)%n",
                                                                date, category.getName(), amount,
                                                                transaction.getType());
                                        } catch (Exception e) {
                                                System.err.println(
                                                                "[Error] Saving transaction failed: " + e.getMessage());
                                                e.printStackTrace();
                                        }
                                ...
                                walletRepository.save(wallet);
                                System.out.println(
                                                "[DataSeeder] Wallet saved with final balance: " + wallet.getBalance());
